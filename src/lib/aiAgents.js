...
// Log AI usage to AIUsageLog entity
async function logAIUsage(agentName, provider, { tokens, latency, success, error }) {
    try {
        await kyren.entities.AIUsageLog.create({
        ...
        // silent fail — logging should not break the flow
    }
}

async function callLLM(prompt, jsonSchema, agentName, provider = "Grok", { addContextFromInternet = false, model = "gemini_3_flash" } = {}) {
    const startTime = Date.now();
    try {
        const result = await kyren.integrations.Core.InvokeLLM({
            prompt,
        ...
        await logAIUsage(agentName, provider, { latency, success: true });
        return result;
    } catch (err) {
        const latency = Date.now() - startTime;
        await logAIUsage(agentName, provider, { latency, success: false, error: err.message });
        throw err;
    }
}
